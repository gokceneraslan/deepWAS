...
if (file.exists('data/permutation_models.Rds')) {
  ...
  U <- mclapply(features.uniq, function(feature) {
    ...
    list(mainfit=fit, permfits=devi)
  }, mc.cores = 12)
...
```{r, eval=F}
main.model.dev.ratio <- sapply(U, function(feature)feature$mainfit$dev.ratio)
qplot(main.model.dev.ratio) + coord_flip()

model.order <- order(main.model.dev.ratio, decreasing = T)
main.model.dev.ratio.sorted <- main.model.dev.ratio[model.order]

D <- sapply(U, function(feature)sapply(feature$permfits, function(permfit)permfit$dev.ratio))
D <- t(D)
D <- D[model.order,] #sort models based on the dev.ratio of the main model
#pheatmap(D, cluster_rows = F)

p.values <- sapply(seq_len(nrow(D)), function(row)mean(D[row,] >= main.model.dev.ratio.sorted[row]))
